...
class Agent:
  ...
  def collectInitialResources(self, board):
    ...
    Takes the current board setup and awards the current player agent
    ...
    for settlement in self.settlements:
      ...
      # Add the resource that the settlement is currently on
      self.resources[settlement.resource] += 1

      # Find all tiles bordering this settlement and
      # take 1 resource of each of the surrounding tile types
      borderingTiles = board.getNeighborTiles(settlement)
...
class GameState:
  ...
  def getLegalActions(self, agentIndex):
    ...
    -----------------------------
    Parameters:
      agentIndex - the player index for which to return the legal actions for

    Returns: a list of action tuples where the first element in the tuple is the action to take
    and the second element in the tuple is any metadata for that action

    Determines which actions are legal and returns a list of action tuples
    ------------------------------
    """
    ...
    # If they can build at least one road
    if agent.canBuildRoad() > 0:
      # To avoid any duplicate roads, we use a set
      validRoads = set()
    ...
    # If they can settle at least once
